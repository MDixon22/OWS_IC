Partial Public Class IC_PutawayNew
        ...
        Try
            ...
            If Not DB.MakeSQLConnection("Warehouse") Then
                ...
                If Not sqlCmdInsert Is Nothing Then
                    ...
                    sqlCmdInsert.Parameters.AddWithValue("@ToBin", "OWFG")
    ...
    '        If Me.txPallet.Text.Length > 0 Then 'MTO Pallet # was entered
    '            If IsNumeric(Me.txPallet.Text) = False Then
    '                Me.lbError.Text = "Pallet # needs to be numbers."
    '                Me.lbError.Visible = True
    '                Common.JavaScriptSetFocus(Page, Me.txPallet)
    '            Else
    '                Dim iPallet As Integer = CInt(Me.txPallet.Text)
    '                'Validate that the pallet scanned does not exist in the system.
    '                If Not DB.MakeSQLConnection("Warehouse") Then
    '                    sqlCmdPallet = DB.NewSQLCommand("SQL.Query.PalletLookup")
    '                    If Not sqlCmdPallet Is Nothing Then
    '                        sqlCmdPallet.Parameters.AddWithValue("@Pallet", iPallet)
    '                        _pallet = sqlCmdPallet.ExecuteScalar()
    '                        sqlCmdPallet.Dispose() : sqlCmdPallet = Nothing
    '                        DB.KillSQLConnection()

    '                        If RTrim(_pallet).Length < 1 Then
    '                            'Pallet does not exist in database and process can continue
    '                            Common.SaveVariable("Pallet", Me.txPallet.Text, Page)
    '                            Me.txCaseLabel.Visible = True
    '                            Me.lbCaseLabel.Visible = True
    '                            Me.lbPrompt.Text = "Scan or Enter Case Label"
    '                            Common.JavaScriptSetFocus(Page, Me.txCaseLabel)
    '                        Else 'Pallet # already in database - display error
    '                            Common.SaveVariable("Pallet", Me.txPallet.Text, Page)
    '                            Me.lbError.Text = "Pallet # entered already exists in database. See Supervisor!"
    '                            Me.lbError.Visible = True
    '                            Common.JavaScriptSetFocus(Page, Me.txPallet)
    '                        End If
    '                    Else
    '                        Common.SaveVariable("Pallet", Me.txPallet.Text, Page)
    '                        Me.lbError.Text = "A Data Error occurred during Pallet validation! Check battery and try again."
    '                        Me.lbError.Visible = True
    '                        Common.JavaScriptSetFocus(Page, Me.txPallet)
    '                    End If
    '                Else
    '                    Common.SaveVariable("Pallet", Me.txPallet.Text, Page)
    '                    Me.lbError.Text = "A Communication Error occurred during Pallet validation! Check battery and try again."
    '                    Me.lbError.Visible = True
    '                    Common.JavaScriptSetFocus(Page, Me.txPallet)
    '                End If
    '            End If
    '        Else 'Pallet Textbox was blank 
    '            Common.SaveVariable("Pallet", "", Page)
    '            Me.txCaseLabel.Visible = True
    '            Me.lbCaseLabel.Visible = True
    '            Me.lbPrompt.Text = "Scan or Enter Case Label"
    ...
    '    Catch ex As Exception
    '        Common.SaveVariable("Pallet", Me.txPallet.Text, Page)
    '        Me.lbError.Text = "Exception Error - " & ex.Message.ToString & " - occurred during Pallet validation! Tell supervisor!"
    '        Me.lbError.Visible = True
    '        Common.JavaScriptSetFocus(Page, Me.txPallet)
    '    Finally
    '        If Not sqlCmdPallet Is Nothing Then
    ...
    Protected Sub btRestart_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btRestart.Click
        'Return to the Main Menu
        Common.SaveVariable("newURL", "~/IC_PutawayNew.aspx", Page)
    End Sub
