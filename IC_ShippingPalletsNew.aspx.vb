...
#Region "Variable Declaration"
    ...
    Public completePressed As Boolean
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If bAddTo = True Then
                ...
                Me.lbShipPalletValue.Text = Common.GetVariable("EditPalletNumber", Page).ToString
    ...
    Public Sub ResetControls()
        Select Case Mode
            ...
            Case ScanModes.PalletMode 'Prepare screen to accept CaseLabel Scan
                ...
                Me.lbPrompt.Text = "Scan or Enter Inventory Pallet Tag."
        ...
        Try 'Must change to look at IC_TmpShipPallets
            ...
            If strPallet.Length > 0 Then
                ...
                sqlString = "Select * from IC_TmpShipPalletsNew Where PalletID = " & CLng(strPallet) & " AND Processed = 'N'"
...
#Region "Temp Table"
    ...
    Public Function WriteTempTableRecord(ByVal pallet As Long, ByVal order As Long, ByVal lineno As Integer, _
        ...
        Dim strSQLInsertTemp As String = "INSERT Into IC_TmpShipPalletsNew (PalletID,OrderID,OrderLine,PP_GTIN,OrderQty,CL_GTIN,CodeDate,Lot,CaseQty,txDateTimeEntered,UserID,ProdVariant,SourcePallet) " & _
        "VALUES (" & pallet & "," & order & "," & lineno & ",'" & ppgtin & "'," & ordqty & ",'" & clgtin & "','" & expire & "','" & lotno & "'," & caseqty & ",'" & dtmScanned & "','" & strUnit & "','" & prodvariant & "'," & srcpallet & ")"
    ...
    Public Sub CleanupTempTableRecords()
        ...
        Try
            strPallet = Me.lbShipPalletValue.Text
            strSqlTempCleanup = "Update IC_TmpShipPalletsNew Set Processed = 'Y' Where PalletID = " & CLng(strPallet)
...
#Region "User Entry"
    ...
    Protected Sub txQuantity_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txQuantity.TextChanged
        ...
        Try
            ...
            Call ValidateEntries(Me.htxOrder.Value, Me.htxProduct.Value, Me.htxPallet.Value)
            ...
            If Me.lbError.Text.Length = 0 Then
                ...
                'Validate quantity is available on the Inventory Pallet entered
                If prodqty > CInt(Common.GetVariable("InvPalletQty", Page).ToString) Then
                    Me.lbError.Text = "Case Quantity entered is more than what is available on Inventory Pallet " & Me.htxPallet.Value & "! If the quantity is available please see supervisor to adjust quantity on the Inventory Pallet, otherwise enter a valid quantity."
                    Me.lbError.Visible = True
                    Common.JavaScriptSetFocus(Page, Me.txQuantity)
                    Exit Sub
                End If

                Dim ppGTIN As String = ""
                Dim OrdLine As Integer = 0
                Dim OrdQty As Integer = 0
                Dim ppVer As String = ""

                If Me.htxProduct.Value.Length = 29 Then
                    ppGTIN = Mid(Me.htxProduct.Value, 3, 14)
                    OrdLine = CInt(Mid(Me.htxProduct.Value, 1, 2))
                    OrdQty = CInt(Mid(Me.htxProduct.Value, 17, 4))
                    ppVer = Mid(Me.htxProduct.Value, 27, 3)
                Else
                    ppGTIN = Mid(Me.htxProduct.Value, 4, 14)
                    OrdLine = CInt(Mid(Me.htxProduct.Value, 1, 3))
                    OrdQty = CInt(Mid(Me.htxProduct.Value, 18, 7))
                    ppVer = Mid(Me.htxProduct.Value, 25, 3)
                End If
                ...
                If Not DB.MakeSQLConnection("Warehouse") Then
                    ...
                    sqlCmdShipPalletSum = DB.NewSQLCommand("SQL.Query.GetShippingPalletSumNew")
                    If Not sqlCmdShipPalletSum Is Nothing Then
                        sqlCmdShipPalletSum.Parameters.AddWithValue("@PalletID", strPallet)
                        sqlCmdShipPalletSum.Parameters.AddWithValue("@CL_GTIN", ppGTIN)
                        sqlCmdShipPalletSum.Parameters.AddWithValue("@OrdLine", OrdLine)
                    ...
                    End If
                Else
                    'Error occurred while validating Shipping Pallet Qty - Check Connection
                    Mode = ScanModes.QuantityMode
                    Common.SaveVariable("8SavedMode", Mode, Page)
                ...
                Dim nPallet As Long = CLng(strPallet) 'Shipping Pallet
                'Dim nOrder As Long = CLng(Me.htxOrder.Value) 'Pick Order
                Dim nOrder As Long = CLng(Right(Me.htxOrder.Value, 9).ToString) 'Pick Order 
                'Dim iLineNo As Integer = CInt(Mid(Me.htxProduct.Value, 1, 3)) 'Pick Product Order Line #
                Dim iLineNo As Integer = OrdLine 'Pick Product Order Line #
                'Dim strProdId As String = Mid(Me.htxProduct.Value, 4, 14) 'Pick Product Item #
                Dim strProdId As String = ppGTIN.ToString 'Pick Product Item #
                'Dim iPickQty As Integer = CInt(Mid(Me.htxProduct.Value, 18, 7)) 'Pick Product Item Qty
                Dim iPickQty As Integer = OrdQty 'Pick Product Item Qty 
                'Dim strProdVariant As String = Mid(Me.htxProduct.Value, 25, 3) 'Pick Product Item Version
                Dim strProdVariant As String = ppVer 'Pick Product Item Version 
                ...
                If WriteTempTableRecord(nPallet, nOrder, iLineNo, strProdId, iPickQty, strPartCode, strCDate, strLotNum, prodqty, strProdVariant, nSrcPallet) = False Then
                    'Error occurred while adding current product to shipping pallet temp table - set Mode to quantity to try again
                    Mode = ScanModes.QuantityMode
                    Common.SaveVariable("8SavedMode", Mode, Page)
                    Me.lbError.Text = "Error occurred while adding Product - " & strProdId & " to Shipping Pallet - " & nPallet & ". Check scanner battery and then try entering case quantity again."
                    Me.lbError.Visible = True
                End If
                ResetControls()
    ...
    Protected Sub txPrinter_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txPrinter.TextChanged
        ...
        Try
            ...
            Else
                Me.lbError.Text = "Printer # needs to be numeric entry!"
                Me.lbError.Visible = True
            ...
            If Common.GetVariable("PrintSecondTime", Page).ToString = "N" Then
                ...
                strPallet = Me.lbShipPalletValue.Text
                SavePallet(strPallet & Right(Me.htxOrder.Value, 9))
            Else
...
#Region "Screen Buttons"
    ...
    Protected Sub btYes_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btYes.Click
        ...
        If Me.lbComplete.Text = "N" Then
            ...
            Else
                ...
                End If
            End If
        ElseIf Me.lbComplete.Text = "Y" Then
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            'If Product GTIN exists in the IC_Exceptions table then we need to prompt user for the Gross Weight
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            If ChkGrossWeightException(CLng(Common.GetVariable("GTIN", Page).ToString)) = True Then
    ...
    Protected Sub btComplete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btComplete.Click
        ...
        Try
            ...
            Dim qtyRemain As Integer = 0
            Dim strSqlQtyChk As String = "SELECT PalletID, OrderLine, CL_GTIN, MAX(OrderQty) AS OrderQty, SUM(CaseQty) AS SumCaseQty FROM IC_TmpShipPalletsNew Where PalletID = " & CLng(strPallet) & " AND Processed = 'N' GROUP BY PalletID, CL_GTIN, OrderLine"
    ...
    Protected Sub btNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btNew.Click
        ...
        ElseIf Me.btNew.Text = "Next Product" Then
            ...
            Me.lbError.Text = "The last case quantity entered for the product added to the Shipping Pallet did not match what was ordered. Press Yes button to accept the quantity difference or the No button to scan another Pallet and add more cases."
            Me.lbError.Visible = True
            Me.btNew.Visible = False
            Me.btYes.Visible = True
            Me.btNo.Visible = True
...
#Region "Custom Processes"
    ...
    Public Sub SavePallet(ByVal oPallet As String) ' Must change to use the IC_TmpShipPallets table on SRV04 
        ...
        strUnit = Common.GetVariable("UserID", Page).ToString
        ...
        Try
            ...
            _dateentered = Now()
            sqlPalletInsert = "insertShipPallet2019 "
            'sqlPalletUpdate = "insertShipPallet2019Step2 "
            sqlPalletUpdate = "insertShipPallet2019Step2New "
                ...
                For Each drShippingPalletSave In dsShippingPalletSave.Tables(0).Rows
                    If Not DB.MakeSQLConnection("Warehouse") Then
                        ...
                        If Not sqlCmdPallet Is Nothing Then
                            ...
                            sqlCmdPallet.Parameters.AddWithValue("@pnSourcePallet", drShippingPalletSave.Item("SourcePallet"))
                    ...
                    If Not DB.MakeSQLConnection("Warehouse") Then
                        ...
                        If Not sqlCmdPalletUpdate Is Nothing Then
                            ...
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pstrProduct", drShippingPalletSave.Item("PP_GTIN"))
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pstrExpiration", drShippingPalletSave.Item("CodeDate"))
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pstrMfgOrd", Mid(drShippingPalletSave.Item("Lot"), 3, 5))
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pstrLot", Mid(drShippingPalletSave.Item("Lot"), 8, 5))
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pnQuantity", CInt(drShippingPalletSave.Item("CaseQty")))
                            sqlCmdPalletUpdate.Parameters.AddWithValue("@pnSourcePallet", drShippingPalletSave.Item("SourcePallet"))
    ...
    Public Sub ValidateEntries(ByVal orderid As String, ByVal product As String, ByVal caselabel As String)
        ...
        Me.lbError.Text = ""
        Me.lbError.Visible = False
        Dim sOrder As String = Right(Me.htxOrder.Value, 9).ToString
        Try
            If sOrder.Length <> CInt(ConfigurationManager.AppSettings.Get("UI.Order.Length").ToString) Then
            ...
            Else
                ...
                If bAddTo = True Then
                    If CLng(sOrder) <> CLng(Common.GetVariable("EditPalletOrderID", Page).ToString) Then
        ...
        If Me.htxProduct.Value.Length = 27 Or Me.htxProduct.Value.Length = 29 Then
            Try
                'Validate GTIN in PickProduct is valid
                ...
                If Not DB.MakeSQLConnection("Warehouse") Then
                    ...
                    If Not sqlCmdValGTIN Is Nothing Then
                        If Me.htxProduct.Value.Length = 29 Then
                            sqlCmdValGTIN.Parameters.AddWithValue("@GTIN", Mid(Me.htxProduct.Value, 3, 14))
                        Else
                        ...
                        Else
                            If Me.htxProduct.Value.Length = 29 Then
                                Common.SaveVariable("GTIN", Mid(Me.htxProduct.Value, 3, 14), Page)
                                Common.SaveVariable("OrderQty", CInt(Mid(Me.htxProduct.Value, 17, 4)), Page)
                                Common.SaveVariable("PickVer", CInt(Mid(Me.htxProduct.Value, 27, 3)), Page)
                            Else
                                Common.SaveVariable("GTIN", Mid(Me.htxProduct.Value, 4, 14), Page)
                        ...
                        If Not sqlCmdPallet Is Nothing Then
                            ...
                            Else
                                If dsPallet.Tables(0).Rows.Count > 0 Then
                                    ...
                                    If _gtin <> Common.GetVariable("GTIN", Page).ToString Then
                                        If Me.htxProduct.Value.Length = 29 Then
                                            Me.lbError.Text = "Pallet scanned is not for the order. The Pick Product GTIN - " & Mid(Me.htxProduct.Value, 3, 14).ToString & " doesn't match the Pallet GTIN - " & Me._gtin.ToString & ". Scan correct pallet or see your supervisor."
                                        Else
                                            Me.lbError.Text = "Pallet scanned is not for the order. The Pick Product GTIN - " & Mid(Me.htxProduct.Value, 4, 14).ToString & " doesn't match the Pallet GTIN - " & Me._gtin.ToString & ". Scan correct pallet or see your supervisor."
                                        End If
    ...
    Public Sub LoadGrid()
        ...
        Try
            strPallet = Me.lbShipPalletValue.Text
            sqlString = "Select ProductID, ProdVariant as Version,CaseQty,ProdDesc from vwIC_TmpShipPalletsGridViewNew Where PalletID = " & CLng(strPallet)
                ...
                If Not sqlCmdShippingPallet Is Nothing Then
                    ...
                    Else
                        If dsShippingPallet.Tables(0).Rows.Count > 0 Then
                            ...
                            Dim strLine8 As String = "HR_OrderID," & Right(Me.htxOrder.Value, 9).ToString
                            ...
                            Else ' The pallet is being shipped to Hickory Farms
                                ...
                                Dim strLineHF1 As String = "*FORMAT,RDC_OW_SHIPTO_HF_NEW4"
