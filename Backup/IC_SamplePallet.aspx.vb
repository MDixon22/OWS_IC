...
Partial Public Class IC_IC_SamplePallet
    ...
    Public Sub ResetControls()
        ...
        Try 'Must change to look at IC_TmpSamplePallets
            ...
            If strPallet.Length > 0 Then
                ...
                sqlString = "Select * from IC_TmpSamplePallets Where PalletID = " & CLng(strPallet)
...
#Region "Temp Table"
    ...
    Public Function WriteTempTableRecord(ByVal pallet As Long, ByVal order As Long, _
        ...
        Me.lbError.Text = ""
        Dim strSQLInsertTemp As String = "INSERT Into IC_TmpSamplePallets (PalletID,OrderID,CL_GTIN,CodeDate,Lot,CaseQty,txDateTimeEntered,UserID,ProdVariant) " & _
        "VALUES (" & pallet & "," & order & ",'" & clgtin & "','" & expire & "','" & lotno & "'," & caseqty & ",'" & dtmScanned & "','" & strUnit & "','" & prodvariant & "')"
    ...
    Public Sub CleanupTempTableRecords()
        ...
        Try
            strPallet = Me.lbSamplePalletValue.Text
            strSqlTempCleanup = "Delete from IC_TmpSamplePallets Where PalletID = " & CLng(strPallet)
...
#Region "User Entry"
    ...
    Protected Sub txQuantity_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txQuantity.TextChanged
        ...
        Try
            ...
            Call ValidateEntries(Me.htxOrder.Value, Me.htxCaseLabel.Value)
            ...
            If Me.lbError.Text.Length = 0 Then
                ...
                Dim strProdVariant As String
                If Me.htxCaseLabel.Value.Length = 44 Then
                    strProdVariant = Mid(Me.htxCaseLabel.Value, 42, 3)
                Else
                    strProdVariant = "000"
                End If

                If WriteTempTableRecord(nPallet, nOrder, strPartCode, strCDate, strLotNum, prodqty, strProdVariant) = False Then
    ...
    Protected Sub txPrinter_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txPrinter.TextChanged
        ...
        Try
            ...
            If Trim(Me.txPrinter.Text).Length = 1 And IsNumeric(Me.txPrinter.Text) = True Then
...
#Region "Custom Processes"
    ...
    Public Sub SavePallet(ByVal oPallet As String) ' Must change to use the IC_TmpSamplePallets table on SRV04 
        ...
        Try
            ...
            _dateentered = Now()
            sqlPalletInsert = "insertSamplePallet "
    ...
    Public Sub ValidateEntries(ByVal orderid As String, ByVal caselabel As String)
        ...
        'Validate Case Label
        If Me.htxCaseLabel.Value.Length <> CInt(ConfigurationManager.AppSettings.Get("UI.CaseLength").ToString) And Me.htxCaseLabel.Value.Length <> CInt(ConfigurationManager.AppSettings.Get("UI.AltCaseLength").ToString) Then
